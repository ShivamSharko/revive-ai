...
def evaluate_consent(db: Session, failure: PaymentFailure, diag: DiagnosisOut):
    ...
    if diag.archetype == "lifecycle" and diag.owner == "merchant":
        return Verdict.BLOCK, "RBI_MANDATE", "Pre-debit notification < 24h. RBI compliance block."

    # LAW 2: Fee Shock Prevention
    if diag.archetype == "intent" and diag.owner == "merchant":
        return Verdict.BLOCK, "FEE_SHOCK", "Hidden fees caused abandonment. Do not retry."

    # LAW 3: Structural Stop (Don't spam broken customers)
    if diag.owner == "customer_structural":
        return Verdict.BLOCK, "STRUCTURAL_STOP", "Repeated failures. Spamming will cause churn."
    ...
    if diag.archetype == "affordability" and diag.owner == "customer_temp":
        if history:
            ...
            return Verdict.DEFER, "LIQUIDITY_DEFER", f"Insufficient funds. Defer to salary day {salary_day}."
        return Verdict.DEFER, "LIQUIDITY_DEFER", "Insufficient funds. Defer to next month."
    ...
    if diag.archetype == "technical" and failure.context == "post_session_offline":
        return Verdict.BLOCK, "OFFLINE_QR_TRAP", "Customer left store. Silent retry blocked to prevent double-charge."

    # LAW 5: Technical / Infra / Merchant Config -> Safe to retry
    if diag.archetype == "technical":
        return Verdict.ALLOW, "TECH_RETRY", "Transient technical failure. Safe to retry."

    # Default
    return Verdict.ALLOW, "DEFAULT_ALLOW", "No blocking rules triggered."